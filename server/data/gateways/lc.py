...
class LCGateway:
    async def search_accounts(self, params: ProfileSearchParams) -> ProfileSearchResults:
        ...
        with bs4_scope(search_results) as parsed_results:
            profiles: List[Profile] = [
                Profile(name=link.text.strip(), value=urlparse(link.get("href")).path)
    ...
    async def search_shelves(self, params: ShelvesSearchParams) -> List[Shelf]:
        ...
        with bs4_scope(search_results) as parsed_results:
            ...
            shelves: List[Shelf] = [
                Shelf(name=shelf_tag['data-shelf-name'], value=shelf_tag['value'])
