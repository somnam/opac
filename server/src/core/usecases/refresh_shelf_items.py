...
class RefreshShelfItemsUseCase:
    ...
    async def execute(self, shelf: Shelf) -> None:
        ...
        gateway_items = await self._repository.gateway.shelf.items(profile, shelf)
        ...
        with self._repository.unit_of_work():
            ...
            if result:
                if result.new:
                    ...
                    self._repository.shelf_item.create_many(new_shelf_items)
                ...
                if result.deleted:
                    ...
                    self._repository.shelf_item.delete_many(deleted_shelf_items)
